...
enum Commands {
    ...
    Client {
        ...
        /// Optional Vault Password (avoids prompt)
        #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Client { address, username, peer_fingerprint, proxy, identity, group, temp, vault_password } => {
            client::run(address, username, peer_fingerprint, proxy, identity, group, temp, vault_password).await?;
        }
    }
